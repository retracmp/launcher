...
export const hasPakInstalled = async (isPlay: bool) => {
  ...
  for (const item in okayFiles) {
    ...
    if (localFileSize !== pakSizes.data[item]) {
      console.log(
        item,
        "is not same size!",
        `CDNSIZE: ${pakSizes.data[item]} LOCALSIZE: ${localFileSize}`
      );
    }
...
export const DownloadCustomContent = async () => {
  ...
  if (paksToDownload.ok) {
    for (const filename of paksToDownload.data) {
      await downloadFile(
        "https://cdn.0xkaede.xyz/data",
        filename,
        `${
          libraryControl.getCurrentEntry()?.path
        }\\FortniteGame\\Content\\Paks\\${filename}`
      );
    }
  }
